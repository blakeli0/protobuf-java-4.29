...
final class FieldSet<T extends FieldSet.FieldDescriptorLite<T>> {
  ...
  public boolean isInitialized() {
    ...
    for (final Entry<T, Object> entry : fields.getOverflowEntries()) {
  ...
  public void mergeFrom(final FieldSet<T> other) {
    ...
    for (final Entry<T, Object> entry : other.fields.getOverflowEntries()) {
  ...
  public void writeTo(final CodedOutputStream output) throws IOException {
    int n = fields.getNumArrayEntries(); // Optimisation: hoist out of hot loop.
    for (int i = 0; i < n; i++) {
      final Entry<T, Object> entry = fields.getArrayEntryAt(i);
  ...
  private void writeMessageSetTo(final Entry<T, Object> entry, final CodedOutputStream output)
      throws IOException {
    final T descriptor = entry.getKey();
    if (descriptor.getLiteJavaType() == WireFormat.JavaType.MESSAGE
  ...
  static final class Builder<T extends FieldDescriptorLite<T>> {
    ...
    private static <T extends FieldDescriptorLite<T>> void replaceBuilders(
        Entry<T, Object> entry, boolean partial) {
